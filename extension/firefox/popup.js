...
async function init() {
  ...
  const localApi = typeof browser !== 'undefined' ? browser.storage.local : chrome.storage.local;
  const stored = await localApi.get(['rollingSegments', 'lastSegments']);
...
async function stopRecording() {
  ...
  if (res?.ok && res.segments > 0) {
    ...
    showToast(`Captured ${res.segments} segment${res.segments > 1 ? 's' : ''}`, 'success');
...
clearBtn.addEventListener('click', async () => {
  ...
  updateStats();
  disableDownloadButtons();
  const clearApi = typeof browser !== 'undefined' ? browser.storage.local : chrome.storage.local;
  await clearApi.remove(['rollingSegments', 'lastSegments', 'lastVTT', 'lastTXT']);
});
...
saveSettingsBtn.addEventListener('click', async () => {
  ...
  await storageApi.set({ meetingScribeSettings: settings });
...
async function loadSettings() {
  const api = typeof browser !== 'undefined' ? browser.storage.sync : chrome.storage.sync;
  const res  = await api.get('meetingScribeSettings');
  return res.meetingScribeSettings ?? {};
}

async function sendBg(msg) {
  // Firefox: browser.runtime.sendMessage returns a real Promise
  // Chrome:  chrome.runtime.sendMessage uses callbacks — wrap in Promise
